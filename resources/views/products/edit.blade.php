                            ...
                            <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                                @foreach(\App\Models\Product::AVAILABLE_SIZES as $size)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="sizes[]" value="{{ $size }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array($size, old('sizes', $product->sizes ?? [])) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                                @foreach(\App\Models\Product::AVAILABLE_COLORS as $color)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="colors[]" value="{{ $color }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ in_array($color, old('colors', $product->colors ?? [])) ? 'checked' : '' }}>
