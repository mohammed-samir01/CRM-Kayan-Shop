<x-app-layout>
                    ...
                    <form action="{{ route('orders.update', $order->id) }}" method="POST">
                        ...
                        <div class="mt-8">
                            ...
                            <div id="items-container">
                                <div class="grid grid-cols-12 gap-3 mb-2 font-medium text-gray-700">
                                    <div class="col-span-4">{{ __('المنتج') }}</div>
                                    <div class="col-span-2">{{ __('المتغير (لون/حجم)') }}</div>
                                    <div class="col-span-2">{{ __('الكمية') }}</div>
                                    <div class="col-span-2">{{ __('السعر') }}</div>
                                    <div class="col-span-2">{{ __('الإجمالي') }}</div>
                                </div>
                                
                                @foreach($order->items as $index => $item)
                                <div class="item-row grid grid-cols-12 gap-3 items-end mt-3" id="row-{{ $index }}">
                                    ...
                                    <div class="col-span-2">
                                        <input type="text" name="items[{{ $index }}][variant]" value="{{ $item->variant }}" placeholder="مث: أحمر" required
                                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    </div>
                                    <div class="col-span-2">
                                    ...
                                    <div class="col-span-2 flex gap-2">
                                        <input type="text" name="items[{{ $index }}][line_total]" value="{{ number_format($item->quantity * $item->unit_price, 2, '.', '') }}" readonly
                                            class="w-full rounded-md border-gray-300 shadow-sm bg-gray-50 focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <script>
        ...
        function addRow() {
            ...
            const newRow = `
                <div class="item-row grid grid-cols-12 gap-3 items-end mt-3" id="row-${rowCount}">
                    <div class="col-span-4">
                    ...
                    <div class="col-span-2">
                        <input type="text" name="items[${rowCount}][variant]" placeholder="مث: أحمر" required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="col-span-2">
